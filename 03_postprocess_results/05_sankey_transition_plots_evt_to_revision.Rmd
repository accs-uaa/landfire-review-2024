...
sankey_by_group_inv <- function(df, group) {
  ...
  sn <- sankeyNetwork(Links = data_long, Nodes = nodes,
                       ...
                       sinksRight=FALSE, colourScale=ColourScal, nodeWidth=40, fontSize=fontsize, nodePadding=nodepadding)
  sn
  saveNetwork(sn, path(wDir, "inverse_transition_plots/html", glue("inv_transition_{group}.html")))

  # library(webshot)
  # you convert it as png
  webshot(path(wDir, "inverse_transition_plots/html", glue("inv_transition_{group}.html")), path(wDir, "inverse_transition_plots", glue("inv_transition_{group}.png")), vwidth = 2000, vheight = 1000)
...
dir_create(path(wDir, "inverse_transition_plots/html"), mode="2775")
# sankey_by_group(evt_x_rev_vat_filt, "Arctic_herbaceous")
