...
if os.path.exists(decratio_output) == 0:
    ...
    with rasterio.open(decratio_output, 'w', **input_profile, BIGTIFF='YES') as dst:
        ...
        for block_index, window in area_raster.block_windows(1):
            ...
            picmar_block = picmar_raster.read(window=window,
                                              masked=False)
            dectre_block = dectre_raster.read(window=window,
                                              masked=False)
            # Calculate Picea ratio
...
if os.path.exists(ndshrub_output) == 0:
    ...
    with rasterio.open(ndshrub_output, 'w', **input_profile, BIGTIFF='YES') as dst:
        ...
        for block_index, window in area_raster.block_windows(1):
            area_block = area_raster.read(window=window,
                                          masked=False)
            alnus_block = alnus_raster.read(window=window,
                                            masked=False)
            salshr_block = salshr_raster.read(window=window,
                                              masked=False)
            betshr_block = betshr_raster.read(window=window,
                                              masked=False)
...
if os.path.exists(eridwarf_output) == 0:
    ...
    with rasterio.open(eridwarf_output, 'w', **input_profile, BIGTIFF='YES') as dst:
        ...
        for block_index, window in area_raster.block_windows(1):
            area_block = area_raster.read(window=window,
                                          masked=False)
            empnig_block = empnig_raster.read(window=window,
                                              masked=False)
            rhoshr_block = rhoshr_raster.read(window=window,
                                              masked=False)
            vacvit_block = vacvit_raster.read(window=window,
                                              masked=False)
...
if os.path.exists(wetland_output) == 0:
    ...
    with rasterio.open(wetland_output, 'w', **input_profile, BIGTIFF='YES') as dst:
        ...
        for block_index, window in area_raster.block_windows(1):
            area_block = area_raster.read(window=window,
                                          masked=False)
            sphagn_block = sphagn_raster.read(window=window,
                                              masked=False)
            wetsed_block = wetsed_raster.read(window=window,
                                              masked=False)
            # Calculate wetland indicator
...
if os.path.exists(picwet_output) == 0:
    ...
    with rasterio.open(picwet_output, 'w', **input_profile, BIGTIFF='YES') as dst:
        ...
        for block_index, window in area_raster.block_windows(1):
            ...
            wetsed_block = wetsed_raster.read(window=window,
                                              masked=False)
            erivag_block = erivag_raster.read(window=window,
                                              masked=False)
            # Calculate Picea mariana wet indicator
...
if os.path.exists(vegetation_output) == 0:
    ...
    with rasterio.open(vegetation_output, 'w', **input_profile, BIGTIFF='YES') as dst:
        ...
        for block_index, window in area_raster.block_windows(1):
            ...
            dectre_block = dectre_raster.read(window=window,
                                              masked=False)
            decshr_block = decshr_raster.read(window=window,
                                              masked=False)
            evrshr_block = evrshr_raster.read(window=window,
                                              masked=False)
            forb_block = forb_raster.read(window=window,
                                          masked=False)
            gramin_block = gramin_raster.read(window=window,
                                              masked=False)
            lichen_block = lichen_raster.read(window=window,
                                              masked=False)
            picgla_block = picgla_raster.read(window=window,
                                              masked=False)
            picmar_block = picmar_raster.read(window=window,
            ...
            sphagn_block = sphagn_raster.read(window=window,
                                              masked=False)
            # Set no data to zero
            decshr_block = np.where(decshr_block == 255, 0, decshr_block)
            evrshr_block = np.where(evrshr_block == 255, 0, evrshr_block)
            forb_block = np.where(forb_block == 255, 0, forb_block)
            gramin_block = np.where(gramin_block == 255, 0, gramin_block)
            lichen_block = np.where(lichen_block == 255, 0, lichen_block)
            # Calculate vegetation sum
            raster_block = dectre_block + decshr_block + evrshr_block + forb_block + gramin_block + lichen_block + picgla_block + picmar_block + sphagn_block
